...
class StripeConnector(BasePaymentConnector):
    ...
    async def charge(
        ...
        if self._is_live:
            logger.info(
                "Stripe live charge: %s %s -> %s (mock fallback)",
                amount, currency, self.params.endpoint_url,
            )
        return ChargeResult(
            ok=True,
            provider_ref=f"MOCK-STRIPE-{uuid4().hex[:12]}",
        )
    ...
    async def refund(
        ...
        if self._is_live:
            logger.info(
                "Stripe live refund: %s %s ref=%s (mock fallback)",
                amount, currency, provider_ref,
            )
        return RefundResult(
            ok=True,
            provider_ref=f"MOCK-STRIPE-REF-{uuid4().hex[:12]}",
        )

    async def test_connection(self) -> ChargeResult:
        if self._is_live:
            logger.info("Stripe test_connection -> %s (mock fallback)", self.params.endpoint_url)
        return ChargeResult(ok=True, provider_ref="MOCK-STRIPE-TEST")
