...
class LedgerEntry(BaseModel):
    ...
    debit_account_id: Mapped[UUID] = mapped_column(ForeignKey("chart_of_accounts.id"))
    debit_amount: Mapped[Decimal] = mapped_column(Numeric(15, 2))
    credit_account_id: Mapped[UUID] = mapped_column(ForeignKey("chart_of_accounts.id"))
    ...
    posted_by: Mapped[UUID | None] = mapped_column(ForeignKey("users.id"), nullable=True)
